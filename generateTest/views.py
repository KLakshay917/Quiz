...
def storeQA(request,quiz,no_of_prevques):
        ...
        q_required=request.POST.getlist('question_required[]')
        print(questions)
        print("requ",q_required)
        print("no",number)
        mark=request.POST.getlist('marks[]')
        l=len(questions)

        for q in range(0,l):
         question=Questions.objects.create(test_id=quiz,name=questions[q],marks=mark[q],q_req=q_required[q])
         print("q",questions[q])
...
def Edit(request,quiz_id):
    ...
    if request.method=='POST':
        ...
        edit_required=request.POST.getlist('edit_question_required[]')
        number=request.POST.getlist('edit_question_number[]')
        print("num",number)
        print("ed",edit_questions)
        print("ed_req",edit_required)
        ...
        for x in range(len(edit_questions)):
                questions.filter(id=edit_qid[x]).update(name=edit_questions[x], marks=edit_mark[x],q_req=edit_required[x])
...
def Details(request,quiz_id):
    ...
    if form.is_valid():
        ...
        if not quiz.mutli_response and Student.objects.filter(email=entered_email):
...
def viewResponses(request,quiz_id):
    ...
    quiz=Test.objects.get(id=quiz_id)
    question=Questions.objects.filter(test_id=quiz_id)
    link=f'https://preogative.in/exam/studentDetails/{quiz_id}' 
    print(student_list)
    ...
    context = {
        ...
        'quiz':quiz,'link':link,'total_questions':len(question)
    }
...
def down_csv(request,quiz_id):
    ...
    question=Questions.objects.filter(test_id=quiz_id)
    writer.writerow(['Total','Questions',len(question)])
    writer.writerow([])
    ...
    for student in students:
        writer.writerow([student.name,student.father_name,student.email,student.dob,student.phone_number,student.marks,student.correct,student.incorrect,student.unanswered,])
